...
class TransferContract extends Contract {
    ...
    _createProductCompositeKey(stub, id) {//don't need composite keys
        return stub.createCompositeKey('Asset',[`${id}`]);
    }

/*     _createBatchId(stub,id){
        return stub.createBatchId('Batch',[`${id}`]);
    } */

    async _getAsset(stub,id){
        const assetBytes=await stub.getState(this._createProductCompositeKey(stub,id));
        if (Buffer.isBuffer(assetBytes)){
            assetBytes=JSON.parse(assetBytes.toString('utf-8'));
        }
        if(assetBytes.is_batch){
    ...
    async _getBatch(stub,id){
        const batchBytes=await stub.getState(this._createProductCompositeKey(stub,id));
        return Batch.from(batchBytes);
    }

    async checkOwnership(ctx,id){
        const productinstance=JSON.parse((await this._getAsset(ctx.stub,id)).toJson());
    ...
    async InitLedger(ctx) {//git something something
        const assets= [
            {
                id:"1234",
    ...
    async CreateProduct(ctx,type,parent_id,location,weight,temperature,usebydate,date_for_id) {
        ...
        const product_id=sha1(JSON.stringify({date:date_for_id,product:asset}));
        asset.id=product_id;
      ...
      /*   const out_result={
        ...
        const out_result={
            ...
            asset_id:asset.id,
            transaction_details:{type:"CreateProduct",from:parent_id,to:product_id},
            user_id : ctx.clientIdentity.getAttributeValue('User_name'),
            org_id : ctx.clientIdentity.getAttributeValue('Organisation'),
        };
    ...
    async CreateBatch(ctx,type,location,weight,temperature,date_for_id){
        ...
        const batch_id=sha1(JSON.stringify({date:date_for_id,product:batch}));
        batch.id=batch_id;
        const exists = await this._AssetExists(ctx, batch_id);

        if (exists) {
            return('Batch already exists!')
            //throw new Error(`The asset ${batch_id} already exists`);
        }

        //const key = [asset.owner_Org,asset.id]

        // we insert data in alphabetic order using 'json-stringify-deterministic' and 'sort-keys-recursive'
        await ctx.stub.putState(this._createProductCompositeKey(ctx.stub,batch_id), Batch.from(batch).toBuffer());


        const out_result={
            _id:ctx.stub.getTxID(),
            asset_id:batch.id,
            transaction_details:{type:"CreateBatch"},
            user_id : ctx.clientIdentity.getAttributeValue('User_name'),
            org_id : ctx.clientIdentity.getAttributeValue('Organisation'),
        };
    ...
    async SplitBatch(ctx,id,weight,temperature,location,date_for_id){

        const checkowner= await this.checkOwnership(ctx, id);

        if (!checkowner){
            return ('User is not authorized to acces this asset');
        }

        const productInstance = await this._getAsset(ctx.stub,id);

        if(weight>productInstance.weight){
            return ('Batching is not allowed')
        }

        productInstance.weight = productInstance.weight-weight;
        
        const batch = {
            owner_Org: ctx.clientIdentity.getAttributeValue('Organisation'),
            is_batch: true,
            parent_id:id,
            transferTo_Org:productInstance.transferTo_Org,  
            type: productInstance.type,
            location:location,
            weight:weight,
            temperature:temperature,
            linkedExperiments:productInstance.linkedExperiments
        };

        await ctx.stub.putState(this._createProductCompositeKey(ctx.stub,id), productInstance.toBuffer());


        const batch_id=sha1(JSON.stringify({date:date_for_id,batch:batch}));
        batch.id=batch_id;
        await ctx.stub.putState(this._createProductCompositeKey(ctx.stub,batch_id), Batch.from(batch).toBuffer());

        // we can do an array of json objects to store these transaction details
        const out_result={
            _id:ctx.stub.getTxID(),
            asset_id:batch_id,
            transaction_details:{type:"Split Batch",from:id,to:batch_id},
            user_id : ctx.clientIdentity.getAttributeValue('User_name'),
            org_id : ctx.clientIdentity.getAttributeValue('Organisation'),
        };
    ...
    async RequestTransfer(ctx,newOwnerOrg,id){
        ...
        if (!exists) {
            //throw new Error(`The asset ${id} does not exist`);
            return (`This asset ${id} does not exist`)
        }
        const checkowner= await this.checkOwnership(ctx, id);
        if (! checkowner){
            //throw new Error('User is not authorized to access this asset');
            return ('User is not authorized to access this asset');
        }
        const productInstance = await this._getAsset(ctx.stub,id);
        if (productInstance.transferTo_Org!=''){
            //throw new Error('A transfer has already been requested for this product')
            return ('A transfer has already been requested for this product');
        }
...
/*         const new_asset_to_put={
    ...
    async TransferComplete(ctx,id,newOwner_user,location,temperature,weight) {//modify to only allow acceptance of transfer when TransferTo attribute matches ctx user identity
        ...
        if (!exists) {
            //throw new Error(`The asset ${id} does not exist`);
            return (`The asset ${id} does not exist`);

        }
        const productInstance=await this._getAsset(ctx.stub,id);
        if(productInstance.transferTo_Org==''){
            //throw new Error('Transfer has not been requested')
            return ('Transfer has not been requested')
        ...
        if (!ctx.clientIdentity.assertAttributeValue('Organisation',productInstance.transferTo_Org)){
            ...
            return ('Organisation does not have permission to complete product transfer')
        ...
        productInstance.linkedExperiments=[];
    ...
    async GetAllAssets(ctx) {
        ...
        while (!result.done) {
            ...
            if(ownership_info==record.owner_Org || ownership_info==record.transferTo_Org){
    ...
    async GetProductHistory(ctx,id){
        ...
        while (!result.done) {
            ...
            if(ownership_info==record.owner_Org||ownership_info=='End%20%Client'){
                allResults.push(record);
            }
            result = await iterator.next();
        }
